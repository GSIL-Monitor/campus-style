...
const userinfo = {
  ...
  async register(ctx) {
    ...
    // 根据用户名得到用户数量
    const userNumResult = await usermodel.getCountByUserName(args);
    ...
    // 插入注册数据
    const userResult = await usermodel.add(args);
  ...
  async login(ctx) {
    ...
    // 根据用户名得到用户信息
    const userResult = await usermodel.getByUserName(args);
